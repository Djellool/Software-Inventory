...
class AppsExport extends DefaultValueBinder implements WithEvents,WithCustomValueBinder,FromCollection,WithHeadings,WithMapping,ShouldAutoSize,WithCustomStartCell,WithColumnFormatting
{
    ...
    {
        $applications = DB::select(DB::raw('SELECT applications.* , U.username,S.nom As structname , R.nom as structutilname FROM (SELECT id ,max(DDM) as dateMAJ FROM applications Group By id) as lastapp
         ...
         Join structures as R On R.id = applications.structutilisatrice_id'));
