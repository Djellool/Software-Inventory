...
class AppController extends Controller
{
    ...
    return view('NewApp',compact('structures'));
    }
    ...
    public  function CreateNew(Request $request){
        ...
        if($appp== null){
            Application::insert([
                ...
                'structutilisatrice_id' => $request['structutilisatrice_id'],
    ...
    public  function Create(Request $request,$id)
    {
                                                        ...
                                                        if ($Application->adbd == $request['adbd']) {
                                                            if ($Application->admetier == $request['admetier']) {
                                                                if ($Application->description == $request['description']) {
                                                                    if($Application->Architecture == $request['architecture']){
                                                                        if($Application->interne == $request['interne']){
                                                                            $boolean = false;
                                                                        }
                                                                    }
                                                                }
        ...
        if ($boolean) {
            ...
            $structutil = structure::where('nom','=',$request['structutilisatrice_id'])->first();
            ...
            if ($app == null or $app->id == $id or $Application->nom == $request['nom']) {
                Application::create([
                    ...
                    'struct_id' => Auth::user()->struct_id,
                    'description' => $request['description'],
                    'structutilisatrice_id' => $structutil->id,
                ]);
    ...
    public function showdetails($id)
    {
        ...
        if($app->struct_id != Auth::user()->struct_id && $admin==false)
        {
            abort(404);
        }
        $user = User::where('id','=',$app->user_id)->first();
        $struct = structure::find($app->struct_id);
        $structutil = structure::find($app->structutilisatrice_id);
        ...
        return view('FicheApp',compact('app','Responsable','user','admin','depusers','struct','structures','structutil'));
    }
    ...
    public function History($id){
        ...
        $app = DB::table('applications')
            ...
            ->orderBy('DDM', 'asc')
            ->get();
        if($app->last()->struct_id != Auth::user()->struct_id &&  Auth::user()->Admin==false)
        {
            abort(404);
        }
        $ap = array(null);
